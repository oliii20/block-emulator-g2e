...
func (tat *TestModule_avgTPS_Broker) UpdateAccountBalance(chain *chain.BlockChain, accounts []string) {
	astates := chain.FetchAccounts(accounts)
	for idx, state := range astates {
		fmt.Printf("[Epoch %d] Account %s Balance: %v\n", tat.epochID, accounts[idx], state.Balance)
	}
}
